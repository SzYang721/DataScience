...
class BO:
    ...
    def __init__(self, X_init: np.ndarray, Y_init: np.ndarray, f: Callable,
        ...
        # ------------------------------------------------------------------------
        # FIXME
        m = Matern(nu=np.inf, length_scale=1.0, variance=1.0)
        # ------------------------------------------------------------------------
    ...
    def __call__(self, step, acquisition: Callable) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # uopdate our gpr class
        self.gpr = self.gpr.update(self.X_sample, self.Y_sample)
        X_next = self.sample_next_point(acquisition, self.xi, self.n_iter)
        k = X_next.shape[0]
        Y_next = self.f(X_next) + np.random.normal(0,self.noise_level,k)

        # xi scheduling here
        step_now = np.linalg.norm(X_next - self.X_sample[-1])
        step_last = np.linalg.norm(X_next - self.X_sample[-2])
        if step_now < step_last:
            self.xi *= 1.6
        else:
            self.xi /= 1.5

        ## update the sample X and Y
    ...
    def sample_next_point(self, acquisition_func: Callable, gpr: object,
        ...
        i = 0
        negate_acq = lambda x: ((-acquisition_func(x, self.X_sample, self.gpr, self.xi)))

        while i < n_restarts:
            bound = tuple(self.bounds[0])
            x0_init =  np.array(np.random.uniform(bound[0], bound[1]))
            opt = minimize(fun = negate_acq, x0 = x0_init, method = methods, bounds = self.bounds)
            acquisition_value = - opt.fun
            if i == 0:
        ...
        return best_x.reshape((n_params, 1))
